...
function injectStylesOnce(): void {
  ...
  style.textContent = `
    ...
    .setup-hub { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 2; width: min(320px, 90vw); box-sizing: border-box; background: linear-gradient(160deg, #211c29 0%, #1a1620 100%); border-radius: 18px; padding: 14px 16px; display: flex; flex-direction: column; align-items: stretch; gap: 10px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.55), inset 0 1px 0 rgba(255, 255, 255, 0.05); overflow-y: auto; }
...
export class SetupScreen {
  ...
  constructor(options: SetupScreenOptions) {
    ...
    if (source) {
      ...
      this.players = source.players.slice(0, this.playerCount);
