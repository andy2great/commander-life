...
export function buildDamageTypeDefs(
  ...
  const types: DamageTypeDef[] = [
    localType('damage', 'Damage', target.color ?? '#948fa3', (delta) =>
      applyDamageDelta(target, delta, undoStack, sound, shake, effects),
    ),
  ];
  ...
  if (!isSelfTarget) {
    ...
    types.push(
      localType('lifelink', 'Lifelink damage', attacker.color ?? '#948fa3', (delta) =>
        applyLifelinkDelta(attacker, target, delta, undoStack, sound, shake, effects),
      ),
    );
  }
  ...
  types.push(
    localType('heal', 'Heal', target.color ?? '#948fa3', (delta) =>
      applyHealDelta(target, delta, undoStack, sound, effects),
    ),
  );
  ...
  types.push({
    ...
    apply: (delta) => applyPoisonDelta(poisonState, target.id, delta, undoStack, shake, effects),
  });
